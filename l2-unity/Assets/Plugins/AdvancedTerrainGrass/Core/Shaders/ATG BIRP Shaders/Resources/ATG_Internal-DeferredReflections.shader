Shader "AdvancedTerrainGrass/Internal-DeferredReflections" {
Properties {
	_SrcBlend ("", Float) = 1
	_DstBlend ("", Float) = 1
}
...
// 	Grass
	occlusion *= saturate(specularIntensity + 0.5);
...
//	Specular Occlusion (Gotanda)
	//ind.specular = env0 * saturate(pow( saturate( dot(worldNormal, -eyeVec)) + occlusion, exp2(-16.0 * g.roughness - 1.0)) - 1.0 + occlusion); //occlusion;
    
    //half3 rgb = UNITY_BRDF_PBS (0, specColor, oneMinusReflectivity, oneMinusRoughness, worldNormal, -eyeVec, light, ind).rgb;
	half3 rgb = BRDF1_ATG_PBS (0, specColor, oneMinusReflectivity, oneMinusRoughness, worldNormal, -eyeVec, light, ind, specularIntensity).rgb;

...
namespace KWS
{
    ...
    public class KW_RenderingPipelineDefines
    {
        ...
        static void UpdateDefines()
        {
            ...
            if (pipeline == PipelineType.Unsupported)
            {
                Debug.LogError(@"Water can't work correctly, as it is impossible to determine the current rendering pipeline (urp/hdrp).
            Check whether you have the correct URP / HDRP settings file. Edit -> ProjectSettings -> Graphics -> HDRP/URP asset");
            }
        }
